// ==================================================
// FILE: MixerActivity.kt — ✅ LAHAT NASA IISA! WALANG HIWALAY NA FILE!
// VERSION: 4.0.0 — Knob + Slider + Toggle lahat dito! TAMA ANG PWEStO NG NUMERO!
// UPDATED: 2026-09-15
// ==================================================
...
class KnobView @JvmOverloads constructor(
    ...
    // ✅ TAMA NA ANG ANGGULO — 270° kabuuan, -50 ↙️ IBABA-KALIWA, +50 ↘️ IBABA-KANAN
    ...
    private val paintText = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        ...
        textSize = 12f; textAlign = Paint.Align.CENTER; isFakeBoldText = true
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.save(); canvas.translate(cx,cy); canvas.scale(knobRadius,knobRadius)
        canvas.drawCircle(0f,0f,1f,paintKnobShine); canvas.restore()
        ...
        for (mark in marks) {
            ...
            val label = when(mark){0->"0",50->"+50",-50->"-50",else->if(mark>0)"+$mark" else "$mark"}
        ...
        canvas.save(); canvas.translate(cx,cy); canvas.rotate(indAngle - 90f)
        val indLen = knobRadius * 0.75f
        val indW = 6f
        val path = Path().apply {
            moveTo(-indW/2f, -indLen*0.3f)
            lineTo(0f, -indLen); lineTo(indW/2f, -indLen*0.3f); close()
        }
        canvas.drawPath(path, paintIndicator); canvas.restore()

        paintText.textSize = 13f; canvas.drawText("GAIN", cx, cy - panelRadius*0.85f, paintText)
        paintValueText.textSize = 14f; canvas.drawText("${value.roundToInt()} dB", cx, cy + panelRadius*0.80f, paintValueText)
    }

    override fun onTouchEvent(e: MotionEvent): Boolean {
        if (e.action == MotionEvent.ACTION_DOWN) { lastTouchY = e.y; return true }
        if (e.action == MotionEvent.ACTION_MOVE) {
            value += (lastTouchY - e.y) / height * 100f * 0.6f
            lastTouchY = e.y; return true
        }
...
// ==================================================
// 🎛️ HORIZONTAL SLIDER — NASA IISA NA!
// ==================================================
class HorizontalSliderView @JvmOverloads constructor(
    ...
    var minValue: Float = -100f; var maxValue: Float = 100f
    var onValueChange: ((Float)->Unit)? = null
    ...
    override fun onTouchEvent(e: MotionEvent): Boolean {
        if (e.action == MotionEvent.ACTION_DOWN || e.action == MotionEvent.ACTION_MOVE) {
            value = minValue + (e.x/width)*(maxValue-minValue); return true
        }
        return super.onTouchEvent(e)
    }
}
...
class ToggleButtonView @JvmOverloads constructor(
    ...
    init {
        ...
        setPadding(24,12,24,12); setBackgroundColor(Color.parseColor("#12121F"))
        layoutParams = LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT)
        dot = View(context)
        val p = LayoutParams(32,32)
        p.leftMargin=0; p.topMargin=0; p.rightMargin=16; p.bottomMargin=0
        dot.layoutParams = p; addView(dot)
        tv = TextView(context); tv.textSize=14f; tv.setTextColor(Color.WHITE); addView(tv)
        setOnClickListener { isOn = !isOn }; update()
    }
    private fun update() {
